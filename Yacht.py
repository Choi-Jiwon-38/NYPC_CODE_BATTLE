...
BID_DIFF_RATE_ST = 0.1   # 경매 입찰 시 입찰 점수 보정 비율
BID_DIFF_RATE_END = 0.03  # 경매 입찰 시 입찰 점수 보정 비율
NUM_RULES = 12
SCORE_HIGH_FULLHOUSE = 22 # Full House가 좋은 선택일지 정하는 임계 점수
SCORE_HIGH_FOK = 18       # Four of Kind가 좋은 선택일지 정하는 임계 점수
NR_END_GAME = 8           # 게임 후반부인지 판단할 남은 Rule 개수의 임계값
...
class Game:
    ...
    def calculate_bid_start(self, dice_a: List[int], dice_b: List[int]) -> Bid:
        ...
        score_diff = int(abs(ev_a - ev_b) * BID_DIFF_RATE_ST)
    ...
    def calculate_bid_end(self, dice_a: List[int], dice_b: List[int]) -> Bid:
        num_to_pick = 5
        group_a, group_b = dice_a + self.my_state.dice, dice_b + self.my_state.dice
        ...
        score_diff = int(abs(score_a - score_b) * BID_DIFF_RATE_END)
    ...
    def calculate_put_start(self) -> DicePut:
        best_put = []
    ...
    def calculate_put_end(self) -> DicePut:
        best_put = []
        max_weight = -1.0
        ...
        for dice in unique_combination:
            for rule in all_rules:
                if self.my_state.rule_score[rule.value] is None:
                    ...
                    # 미래 가치 계산
                    future_value = (next_round_ev_table[next_state.mask][next_state.upper_score] * FUTURE_RATE)
                    ...
                    if DEBUG_MODE: print(f"{self.ev_state.round+1}R, rule: {rule}, dice: {sorted(dice)}, scores: ({score}, {bonus_score}, {future_value}), current_ev: {current_ev}", file=sys.stderr)
...
def main():
    ...
    while True:
        try:
            ...
            remaining_rules = sum(1 for s in game.my_state.rule_score if s is None)
            ...
            if command == "ROLL":
                ...
                if remaining_rules <= NR_END_GAME:
                    my_bid = game.calculate_bid_end(dice_a, dice_b)
                else:
                    # 경매 시작 -> 라운드 초기화
                    game.initialize_importance()
                    my_bid = game.calculate_bid_start(dice_a, dice_b)
            ...
            if command == "SCORE":
                # 주사위 골라서 배치하기
                if remaining_rules <= NR_END_GAME:
                    put = game.calculate_put_end()
                else:
                    put = game.calculate_put_start()
